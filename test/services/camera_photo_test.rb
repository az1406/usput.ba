...
class CameraPhotoTest < ActiveSupport::TestCase
  test "a HEIC upload becomes a JPEG the app accepts" do
    upload = uploaded("camera.heic", "image/heic") { |path| write_heic(path) }

    converted = CameraPhoto.as_jpeg(upload)

    assert_equal "image/jpeg", converted.content_type
    assert_equal "camera.jpg", converted.original_filename
    assert_includes Moment::ACCEPTABLE_PHOTO_TYPES, converted.content_type
  end
  ...
  def write_heic(path)
    Vips::Image.black(64, 64, bands: 3).add(128).cast(:uchar).write_to_file(path)
  end
